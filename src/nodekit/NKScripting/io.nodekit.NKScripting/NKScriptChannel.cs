...
namespace io.nodekit.NKScripting
{
    public class NKScriptChannel : NKScriptMessageHandler
    {
        ...
        private async Task<string> getResource(string name, string folder)
        {
            string source;
            ...
            var stream = this.GetType().GetTypeInfo().Assembly.GetManifestResourceStream(resourceNamespace + "." + folder + "." + name);
