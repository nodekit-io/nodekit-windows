...
namespace io.nodekit.NKCore
{
    public class Main
    {
        ...
        public async static Task bootCore(Type entry, NKScriptContext context, Dictionary<string, object> options)
        {
            entryType = entry;
            // FINALLY LOAD BOOTSTRAP CODE AFTER ALL PLATFORM PLUGINS
            var appjs = await NKStorage.getResourceAsync(typeof(Main), "_nodekit_bootstrapper.js", "lib_core");
